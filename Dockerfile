...
RUN curl -o android-sdk.tgz http://dl.google.com/android/android-sdk_r24.4.1-linux.tgz && \
    tar -C /usr/local -xvf android-sdk.tgz

RUN echo y | android update sdk --all --filter build-tools-22.0.1,android-22,sys-img-x86_64-android-22,platform-tool  --no-ui --force
